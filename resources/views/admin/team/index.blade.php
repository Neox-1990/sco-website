...
@section('content')
  ...
  <h3 class="mt-5 teamtabletoggle">{{$name}} ({{sizeof($list['pending'])}}/{{sizeof($list['waitinglist'])}}/{{sizeof($list['reviewed'])}}/{{sizeof($list['confirmed'])}})<span class="ml-3 toggle-icon"><i class="fa fa-chevron-circle-down closed" aria-hidden="true"></i></span></h3>
  <div class="">
      ...
      <thead>
        ...
        @foreach ($list['reviewed'] as $team)
          <tr>
            ...
            <td><form class="" action="{{url('admin/teams/'.$team['id'])}}" method="post">
              {{csrf_field()}}
              <input class="btn btn-info" type="submit" name="review" value="set review" disabled>
            </form></td>
            <td><form class="" action="{{url('admin/teams/'.$team['id'])}}" method="post">
              {{csrf_field()}}
