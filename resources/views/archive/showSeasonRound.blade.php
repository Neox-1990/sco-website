...
<div class="row mx-0">
  <div class="col-12" style="padding-bottom:2rem;">
    <h1>{{$season['name']}}</h1>
    <hr>
    <a href="{{url('/archive/'.$season['id'])}}" class="btn btn-outline-primary m-2">Championship</a>
    <div class="mb-3 d-flex flex-wrap justify-content-start align-items-stretch" id="results_controlbox_rounds">
      @foreach ($rounds as $roundX)
        <a href="{{url('/archive/'.$season['id'].'/'.$roundX->id)}}" class="btn btn-{{$roundX->id==$round->id?'primary':'outline-primary'}} m-2">Round {{$roundX->number}}</a>
      @endforeach
    </div>
